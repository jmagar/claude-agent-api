...
- **CRITICAL**: `get_agent_service()` in `dependencies.py` doesn't pass `mcp_config_injector` parameter when creating `AgentService` instance
  - **Impact**: Application config file never loaded, three-tier merge never invoked
  - **Resolution**: Update `get_agent_service()` to accept `McpConfigInj` dependency and pass to `AgentService()`
  - **Discovered**: Task 1.14 POC checkpoint (manual E2E test)

## Next

Fix dependency injection blocker, then proceed to Phase 2 (TDD)
