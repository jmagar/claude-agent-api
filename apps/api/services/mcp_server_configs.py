...
class McpServerConfigService:
    ...
    def _server_key(self, name: str) -> str:
        """<summary>Build cache key for an MCP server.</summary>"""
        return f"mcp_server:{name}"

    async def list_servers(self) -> list[McpServerRecord]:
        """<summary>List all MCP servers.</summary>"""
        names = await self._cache.set_members(self._INDEX_KEY)
        if not names:
            return []

        keys = [self._server_key(name) for name in names]
    ...
    async def get_server(self, name: str) -> McpServerRecord | None:
        """<summary>Get MCP server config by name.</summary>"""
    ...
    async def delete_server(self, name: str) -> bool:
        """<summary>Delete MCP server config.</summary>"""
        await self._cache.remove_from_set(self._INDEX_KEY, name)
